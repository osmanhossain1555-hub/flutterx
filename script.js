...
function createEmptyBudgets() {
  return Object.fromEntries(Object.keys(DEFAULT_BUDGETS).map((name) => [name, 0]));
}
...
function normalizeState(data) {
  const empty = createEmptyState();
  const mergedBudgets = { ...createEmptyBudgets(), ...(data?.budgets || {}) };
  const selectedCategory = mergedBudgets[data?.selectedCategory] !== undefined
    ? data.selectedCategory
    : "Internet Bills";
  return {
    monthlyIncome: Number(data?.monthlyIncome ?? empty.monthlyIncome),
...
function bindEvents() {
  ...
  document.getElementById("budgetResetBtn").addEventListener("click", resetAppData);
  document.getElementById("menuBtn").addEventListener("click", openMenuDrawer);
  document.getElementById("closeDrawerBtn").addEventListener("click", closeMenuDrawer);
  document.querySelector("[data-close-drawer='true']").addEventListener("click", closeMenuDrawer);
  els.drawerActions.addEventListener("click", handleDrawerClick);
  ...
  document.getElementById("notifyBtn").addEventListener("click", openNotificationsSheet);
  document.getElementById("closeInfoSheetBtn").addEventListener("click", closeInfoSheet);
  document.querySelector("[data-close-info-sheet='true']").addEventListener("click", closeInfoSheet);
...
function resetAppData() {
  const confirmed = window.confirm("Clear all saved income, budgets, and transactions?");
  if (!confirmed) return;
...
async function installApp() {
  ...
  showInfoSheet("Install app", buildInstallHelpHtml());
}
...
function updateInstallButton() {
  ...
  els.installBtn.textContent = deferredPrompt ? "Install" : "How";
}
...
function handleDrawerClick(event) {
  ...
  if (["home", "budget", "plan", "settings"].includes(action)) {
    switchScreen(action);
    closeMenuDrawer();
    return;
  }
...
function showInfoSheet(title, bodyHtml) {
  els.infoSheetTitle.textContent = title;
  els.infoSheetBody.innerHTML = bodyHtml;
...
function openNotificationsSheet() {
  showInfoSheet("Notifications", buildNotificationsHtml());
}
  ...
  const items = [
    {
      ...
      body: `Spent ${formatCurrency(summary.expenses)} out of ${formatCurrency(summary.netIncome || state.monthlyIncome)}.`
    },
    {
      title: "Budget health",
      body: overspent.length
        ? `Over budget in ${overspent.map((item) => item.category).join(", ")}.`
        : "No categories are over budget right now."
    },
    {
      title: "Saved entries",
      body: `${state.transactions.length} transaction${state.transactions.length === 1 ? "" : "s"} stored on this device.`
    },
    {
      title: "App install",
      body: isStandalone()
        ...
        : deferredPrompt
          ? "Install prompt is ready in Settings."
          : "Use the Install button in Settings for steps if the prompt is not available yet."
    }
  ];

  return `<div class="sheet-list">${items.map((item) => `
    <article class="sheet-item">
      <div class="sheet-item-title">${escapeHtml(item.title)}</div>
      <div class="sheet-item-text">${escapeHtml(item.body)}</div>
    </article>
  `).join("")}</div>`;
}

function buildInstallHelpHtml() {
  ...
  if (platform === "android-chrome") {
    ...
    steps = [
      "Open this app from a secure URL (HTTPS) or localhost.",
      "Use Chrome or Edge for the best install support.",
      "Open the browser menu and choose Install app when available."
    ];
  }

  const extra = location.protocol === "file:"
    ? "Install prompts do not work from a local file. Run the app from localhost or a hosted HTTPS site first."
    : "If no prompt appears yet, refresh once and wait a few seconds after the app fully loads.";

  return `
    <div class="sheet-copy">Direct prompt is not available right now, but you can still install it manually.</div>
    <ol class="sheet-steps">${steps.map((step) => `<li>${escapeHtml(step)}</li>`).join("")}</ol>
    <div class="sheet-note">${escapeHtml(extra)}</div>
  `;
}
...
function detectInstallPlatform() {
  ...
  if (/iPhone|iPad|iPod/i.test(ua) && /Safari/i.test(ua) && !/CriOS|FxiOS/i.test(ua)) return "ios-safari";
  return "desktop";
}
