...
const storageKey = 'finance-tracker-pwa-demo';
let state = loadData();
let deferredPrompt = null;

function loadData() {
  const fromStorage = localStorage.getItem(storageKey);
  return fromStorage ? JSON.parse(fromStorage) : structuredClone(demoData);
}
...
function renderHeader() {
  ...
  const incomePct = Math.min(100, Math.round((totals.income / state.budget.incomeTarget) * 100));
  const expensePct = Math.min(100, Math.round((totals.expense / state.budget.total) * 100));
...
function renderPlanner() {
  ...
  planner.innerHTML = '';
  [
    { name: 'Salary', date: 'Aug 10', amount: 6100, cls: 'pill' },
    { name: 'Interest income', date: 'Aug 15', amount: 400, cls: 'pill' },
    { name: 'Cafe', date: 'Aug 18', amount: 74, cls: 'pill yellow' }
  ].forEach(item => {
    const row = document.createElement('div');
...
function renderBudget() {
  ...
  state.budget.bars.forEach((value, index) => {
    ...
    item.innerHTML = `<span style="height:${value}%"></span>`;
